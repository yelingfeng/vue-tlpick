...
// CONCATENATED MODULE: ./src/components/tlpick.js
...
var drawUtils = {
   ...
   * @param p [x,y]
   */
  getTopLineCycle: function getTopLineCycle(p) {
   ...
   * 创建线
   */
  _getLine: function _getLine(p1, p2, color) {
   ...
   * @param p2 [x2 y2]  结束点
   */
  getTopLine: function getTopLine(p1, p2) {
    var line = this._getLine(p1, p2, this.lineTopScaleColor);

    TweenMax.from(line, 0.3, {
      scaleY: 0
    });
    return line;
  },
   ...
   *  @param  p
   */
  getTopLineLeftText: function getTopLineLeftText(name, p) {
    var text = this._getTLineText(name, p);

    text.textAlign = 'left';
    text.textBaseline = 'bottom';
    TweenMax.from(text, 0.3, {
      alpha: 0,
      x: '+=20'
    });
    return text;
   ...
   * @param  p
   */
  getTopLineRightText: function getTopLineRightText(name, p) {
    var text = this._getTLineText(name, p);
   ...
   * @param  p2   [x2 y2]  结束点
   */
  getBottomLine: function getBottomLine(p1, p2) {
    var line = this._getLine(p1, p2, this.lineTopScaleColor);

    TweenMax.from(line, 0.2, {
      scaleY: 0
    });
    return line;
  },
   ...
   * @param  p  [x ,y]
   */
  getBottomLineText: function getBottomLineText(name, p) {
    var text = this._getTLineText(name, p);

    text.textAlign = 'center';
    text.color = '#999'; // 在滑块之外的刻度  透明 50%
   ...
   * @param p [x,y]
   */
  _getTLineText: function _getTLineText(t, p) {
  ...
  isFirstQuarterOfHour: function isFirstQuarterOfHour(date) {
    return date.getMinutes() < 15;
  },
  // 一小时里第一个5分钟刻度
  isFisrtFiveOfHour: function isFisrtFiveOfHour(date) {
    return date.getMinutes() < 5;
  },
  isFirstHourOfDay: function isFirstHourOfDay(date) {
   ...
   * @param type 刻度类型
   */
  isNeedProminentLine: function isNeedProminentLine(beginTime, type) {
    var flag = false;
...
Date.prototype.add = function (interval, value) {
  ...
  switch (interval.toLowerCase()) {
    ...
    case 'month':
      ...
      var lastDayOfPrevMonth = new Date(new_date.getTime() - 1000 * 60 * 60 * 24).getDate();
...
Date.prototype.format = function (pattern) {
  var fullyear = this.getFullYear(),
      minyear = fullyear.toString().substring(2),
  ...
  return pattern.replace('yyyy', fullyear).replace('yy', minyear).replace('MM', month).replace('dd', day).replace('hh', hour).replace('mm', minute).replace('ss', second).replace('SSS', ms);
};
...
var tlpick_TimelinePick =
/*#__PURE__*/
function () {
  function TimelinePick(opts) {
    _classCallCheck(this, TimelinePick);
  ...
  _createClass(TimelinePick, [{
    ...
    value: function init() {
      ...
      this.canvas.appendTo(this.box).attr({
    ...
    value: function create() {
      ...
      this.paddingX = 30;
      this.xScale = d3_default.a.scale.linear().domain([0, this.options.number]).range([0, this.stage.xp - this.paddingX * 2]);
    ...
    key: "_createTopScale",
    value: function _createTopScale(time, type, format, scale) {
    ...
    key: "_createCodeline",
    value: function _createCodeline() {
    ...
    value: function drawCore() {
      ...
      for (var i = 0; i < me.options.number; i++) {
      ...
      for (var _i = 0; _i < me.options.number; _i++) {
        ...
        var line = drawUtils.getBottomLine([me.xScale(_i), 0], [me.xScale(_i), linePointY]);

        if (_i < me.options.spanIndex.start || _i >= me.options.spanIndex.end) {
          line.alpha = 0.5;
        }
        ...
        if (QUARTER === type) {
          if (drawUtils.isFirstQuarterOfHour(beginTime)) {
            var text = drawUtils.getBottomLineText(beginTime.format('hh'), [me.xScale(_i), textPointY]);
          ...
          if (drawUtils.isFisrtFiveOfHour(beginTime)) {
            var _text2 = drawUtils.getBottomLineText(beginTime.format('hh'), [me.xScale(_i), textPointY]);

            if (_i < me.options.spanIndex.start || _i >= me.options.spanIndex.end) {
              _text2.alpha = 0.5;
            } else {
              _text2.color = drawUtils.spanRangeTextColor;
            }

            bottomScale.addChild(_text2);
          }

          beginTime = beginTime.add('minute', 5);
        } else if (HOUR === type) {
          if (_i % 2 === 0) {
            var _text3 = drawUtils.getBottomLineText(beginTime.format('hh'), [me.xScale(_i), textPointY]);

            if (_i < me.options.spanIndex.start || _i >= me.options.spanIndex.end) {
              _text3.alpha = 0.5;
            } else {
              _text3.color = drawUtils.spanRangeTextColor;
            }

            bottomScale.addChild(_text3);
          }

          beginTime = beginTime.add('hour', 1);
        } else {
          if (_i % 2 === 0) {
            var _text4 = drawUtils.getBottomLineText(beginTime.format('dd'), [me.xScale(_i), textPointY]);

            bottomScale.addChild(_text4);

            if (_i < me.options.spanIndex.start || _i >= me.options.spanIndex.end) {
              _text4.alpha = 0.5;
            } else {
              _text4.color = drawUtils.spanRangeTextColor;
            }
          }
    ...
    key: "_createSlider",
    value: function _createSlider(spanWidth, spanHeight) {
      var me = this;
      var slider = new createjs.Shape();
      slider.alpha = 0.75;
      slider.drawColor = me.options.spanColor;
      slider.graphics.beginFill(slider.drawColor).drawRect(0, 0, spanWidth, spanHeight).endFill();
      slider.cursor = 'pointer';
      slider.on('mouseover', function () {
        this.graphics.clear();
        var hoverColor = d3_default.a.rgb(this.drawColor).brighter();
      ...
      slider.on('pressup', function (e) {
        ...
        me.options.spanIndex.start = newIndex; //计算好位置, 重绘
    ...
    key: "_createSliderBtn",
    value: function _createSliderBtn(size) {
      var me = this;
    ...
    value: function drawSlider() {
      ...
      var spanWidth = me.spanDis * (me.options.spanIndex.end - me.options.spanIndex.start - 1); //先画一个区域
      ...
      leftButton.on('pressmove', function (e) {
        if (me.isFixedDrag()) return; //计算临界情况
      ...
      rightButton.on('pressup', function (e) {
        ...
        var newIndex = (positionIndex * 10 % 10 || 0) < 5 ? positionIndex || 0 : (positionIndex || 0) + 1;
        me.options.spanIndex.end = newIndex + 1; //计算好位置, 重绘
    ...
    key: "_createArrow",
    value: function _createArrow(vector, p1, p2, p3) {
      var me = this;
    ...
    value: function drawArrow() {
      ...
      this.leftArrow = this._createArrow.apply(this, ['left'].concat(leftPath));
      this.rightArrow = this._createArrow.apply(this, ['right'].concat(rightPath));
      this.stage.addChild(this.leftArrow);
      this.stage.addChild(this.rightArrow);
    }
  }, {
    key: "isFixedDrag",
    value: function isFixedDrag() {
      return this.options.isFixedDrag === '0';
    }
  }, {
    key: "isFreeDrag",
    value: function isFreeDrag() {
    ...
    key: "clearStopCaculate",
    value: function clearStopCaculate() {
    ...
    key: "refreshCaculate",
    value: function refreshCaculate() {
      this.drawCore();
      this.drawSlider();
      this.options.callback(this.caculateTimeSpan());
    }
  }, {
    key: "caculateTimeSpan",
    value: function caculateTimeSpan() {
      var me = this;
      var timeType = me.options.type === 'date' ? 'date' : me.options.type;
      var dateFormat = me.options.dateFormat; // 刻

      if (me.options.type === QUARTER) {
        var baseTime = me.nowTime.add('minute', 15 * (1 - me.options.number));
        var startTime = baseTime.add('minute', 15 * me.options.spanIndex.start);
        var endTime = baseTime.add('minute', 15 * (me.options.spanIndex.end - 1));
        ...
        return {
          ...
          var _baseTime = me.nowTime.add('minute', 5 * (1 - me.options.number));

          var _startTime = _baseTime.add('minute', 5 * me.options.spanIndex.start);

          var _endTime = _baseTime.add('minute', 5 * (me.options.spanIndex.end - 1));

          var _ts = _startTime.getMinutes() / 5 || 0;

          var _te = _endTime.getMinutes() / 5 || 0;

          return {
            startTime: _startTime.format('yyyy-MM-dd hh:') + (_ts * 5 < 10 ? '0' + _ts * 5 : _ts * 5) + ':00',
            endTime: _endTime.format('yyyy-MM-dd hh:') + (_te * 5 < 10 ? '0' + _te * 5 : _te * 5) + ':00',
            timeType: timeType
          };
        } // 小时
        else if (me.options.type === HOUR) {
            var _baseTime2 = me.nowTime.add('hour', 1 - me.options.number);

            var _startTime2 = _baseTime2.add('hour', me.options.spanIndex.start);

            var _endTime2 = _baseTime2.add('hour', me.options.spanIndex.end - 1);

            return {
              startTime: _startTime2.format('yyyy-MM-dd hh:00:00'),
              endTime: _endTime2.format('yyyy-MM-dd hh:00:00'),
              timeType: timeType
            };
          } // 天
          else if (me.options.type === DATE) {
              var _baseTime3 = me.nowTime.add('day', 1 - me.options.number);

              var _startTime3 = _baseTime3.add('day', me.options.spanIndex.start);

              var _endTime3 = _baseTime3.add('day', me.options.spanIndex.end - 1);

              var _format = dateFormat !== 'yyyy-MM-dd hh:mm:ss' ? dateFormat : 'yyyy-MM-dd 00:00:00';

              return {
                startTime: _startTime3.format(_format),
                endTime: _endTime3.format(_format),
                timeType: timeType
              };
            }
    }
...
function install(_Vue) {
  _Vue.component('tlpick', tlpick_TimelinePick);
}
...
/******/ });
//# sourceMappingURL=vue-tlpick.common.js.map